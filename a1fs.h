...
typedef struct a1fs_superblock {
	...
	/** Total number of inodes (known when calling mkfs) */
	uint32_t num_inodes;

	// number of available inodes
	uint32_t available_inodes;

	// number of available blocks
	uint32_t available_blocks;

	// address of inode bitmap's block
	uint64_t inode_bitmap;

	// length of inode bitmap
	uint32_t inode_bitmap_length;

	// address of data bitmap's block
    uint64_t data_bitmap;

    // length of data bitmap
    uint32_t data_bitmap_length;

	// address of inode table's block
	uint64_t inode_table;

    // length of inode table
    uint32_t inode_table_length;

	// inode number of the root directory
	a1fs_ino_t root_directory_inode;
...
/** maximum number of extents per file/directory */
#define A1FS_MAX_EXT_NUM 512
...
typedef struct a1fs_inode {
	...
	/** File size in bytes. */
    // if it's a directory, the size is just the size of directory entries
	uint64_t size;
	...
	uint64_t indirect_pt; // points to one data block(512 extent structs: 512 * 8 = 4096)
//	a1fs_ino_t indirect_pt_2;

	/** Number of directory entries */
    uint32_t num_dir_entry; // 0 if mode is a regular file or empty directory
